...
export default function Skills() {
  ...
  const currentCategory = skills.find(
      ...
      <div className="container-custom relative z-10" ref={containerRef}>
            ...
            {currentCategory && (
              ...
              >
                {currentCategory.items.map((skill, index) => {
                  const config =
                    proficiencyConfig[skill.proficiency as ProficiencyLevel];
                  return (
                    <motion.div
                      key={skill.name}
                    ...
                    >
                      ...
                      >
                        {/* Card glow effect */}
                        <div
                          className={`absolute inset-0 bg-gradient-to-r ${config.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500 rounded-3xl`}
                        />
                        ...
                        <div className="relative z-10">
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className=" relative w-12 h-12 bg-gray-700 overflow-hidden rounded-2xl transform group-hover:scale-110 transition-transform duration-300">
                                {/* {skill.icon} */}
                                <Image
                                  src={skill.image}
                                  alt={skill.name}
                          ...
                          {/* Proficiency Badge */}
                          <div
                            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r ${config.color} text-white text-sm font-medium mb-4`}
                          >
                            {skill.proficiency}
                          </div>
                        ...
                        <div className="absolute top-0 right-0 w-20 h-20 overflow-hidden">
                          <div
                            className={`absolute -top-2 -right-2 w-8 h-8 bg-gradient-to-r ${config.color} rounded-full opacity-20 group-hover:opacity-40 transition-opacity duration-300`}
                          />
                        </div>
                      </div>
                    </motion.div>
        ...
        >
          <motion.div
            ...
            transition={{ duration: 0.8, type: "spring" }}
          >
            <h2 className="text-2xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-white via-blue-100 to-cyan-100 bg-clip-text text-transparent">
              Other Skills
            </h2>
            <motion.div
              className="w-32 h-1 bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 mx-auto mb-4 rounded-full"
              initial={{ opacity: 0, scaleX: 0 }}
              animate={
                isInView ? { opacity: 1, scaleX: 1 } : { opacity: 0, scaleX: 0 }
              }
          ...
          {[
            {
              ...
              icon: "🧩",
              desc: "Complex challenge resolution",
            },
            {
